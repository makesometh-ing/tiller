...
These are common mistakes. Read before using the CLI.

### Priority is numeric, not named

`--priority` takes an integer 1–4, not a string:
...
```bash
# Correct
linear issue create -t "Fix crash" --priority 1

# Wrong — will error
linear issue create -t "Fix crash" --priority "urgent"
```

### Always pass `--no-interactive` for automated issue creation

Without it, the CLI may prompt for interactive input and hang. Always include it when creating or updating issues programmatically:

```bash
linear issue create -t "Title" --description-file /tmp/desc.md --no-interactive
```

### `issue list` defaults to `unstarted` state only

Newly created issues (without `--start`) land in **Backlog**, which `issue list` hides by default. To see them:

```bash
linear issue list --all-states              # Everything
linear issue list -s backlog -s unstarted   # Backlog + unstarted
```

### Use the CLI for issue relations — not GraphQL

The CLI has `issue relation add` with clear semantics. Don't reach for the GraphQL API:

```bash
# "TILLER-94 blocks TILLER-95"
linear issue relation add TILLER-94 blocks TILLER-95

# "TILLER-95 is blocked by TILLER-94" (equivalent)
linear issue relation add TILLER-95 blocked-by TILLER-94

# Other relation types: related, duplicate
